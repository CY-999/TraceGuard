...
def estimate_fisher_importance(
    ...
    *,
    ...
    isolated_model = deepcopy(model).to(device)
    isolated_model.train()
    criterion = nn.CrossEntropyLoss()
    trainable_parameters = {
        name: parameter
        for name, parameter in isolated_model.named_parameters()
        if parameter.requires_grad
    }
    fisher = {
        key: torch.zeros_like(parameter, dtype=torch.float32, device="cpu")
    ...
    for images, labels in dataloader:
        ...
        isolated_model.zero_grad(set_to_none=True)
        loss = criterion(isolated_model(images), labels)
        loss.backward()

        for key in fisher:
            parameter = trainable_parameters.get(key)
...
def cluster_or_score_clients_by_discrepancy(
    ...
    *,
    ...
    """Return discrepancy scores and FDCR client weights."""
    scores = compute_fisher_discrepancy(importances, metric=metric)
    median = scores.median()
    mad = (scores - median).abs().median()
    robust_scores = (scores - median) / (mad + float(eps))
    weights = torch.clamp(1.0 - robust_scores / float(tau), min=0.0, max=1.0)
    ...
    *,
    ...
    for key, value in update.items():
        if key not in cohort_importance:
            # Fisher profiles are parameter-only. Floating buffers and
            # non-floating metadata are carried through unchanged.
...
class FDCRDefense:
    ...
    def from_config(cls, config: dict) -> "FDCRDefense":
        fdcr_cfg = config.get("fdcr", {})
        return cls(
            fisher_batches=int(fdcr_cfg.get("fisher_batches", 1)),
            discrepancy_metric=str(fdcr_cfg.get("discrepancy_metric", "cosine")),
            tau=float(fdcr_cfg.get("tau", 4.0)),
            rescale_strength=float(fdcr_cfg.get("rescale_strength", 0.5)),
        )
