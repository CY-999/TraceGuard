...
def _validate_multi_krum_condition(*, n: int, f: int) -> None:
    if f < 0:
        raise ValueError(f"Multi-Krum num_byzantine must be non-negative, got f={f}")
    if n <= 2 * f + 2:
        raise ValueError(
            "Multi-Krum condition not satisfied: "
            f"n={n}, f={f}; require n > 2f + 2. "
            "Increase federated.clients_per_round or decrease multi_krum.num_byzantine."
        )
...
def multi_krum(
    ...
    _validate_multi_krum_condition(n=n, f=f)
