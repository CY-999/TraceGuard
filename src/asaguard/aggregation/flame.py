...
def _fit_agglomerative_precomputed(distance_matrix, n_clusters: int):  # noqa: ANN001
    """Fit agglomerative clustering across sklearn metric/affinity APIs."""
    try:
        from sklearn.cluster import AgglomerativeClustering
    except Exception as exc:  # pragma: no cover - import environment dependent
        raise RuntimeError(f"FLAME clustering failed: cannot import sklearn AgglomerativeClustering: {exc}") from exc
    ...
    except TypeError as metric_exc:
        try:
            model = AgglomerativeClustering(
                n_clusters=int(n_clusters),
            ...
            raise RuntimeError(
                "FLAME clustering failed with both metric='precomputed' and "
                f"affinity='precomputed': {affinity_exc}"
            ) from metric_exc
    except Exception as exc:
        raise RuntimeError(f"FLAME clustering failed: {exc}") from exc
...
def cluster_updates(
    ...
    *,
    ...
    if len(updates) <= 2:
        # With at most two updates there is no meaningful majority cluster.
        return [idx for idx in range(len(updates))]
    ...
    labels = _fit_agglomerative_precomputed(distances, n_clusters=2)
    ...
    if not selected:
        raise RuntimeError("FLAME clustering failed: largest cluster selection was empty")
    return selected
